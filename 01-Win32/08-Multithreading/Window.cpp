...
#include <tchar.h>									//_tcslen() - t char str length
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) //wparam-16 bit, lparam-32 bit. wparam & lparam of every msg are different 
{
	...
	static HANDLE hThread1 = NULL;					//For using local variables across msgs it should be either static or global
	static HANDLE hThread2 = NULL;
	//code
	switch (iMsg)
	{
	case WM_CREATE:
		hThread1 = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ThreadProcOne, (LPVOID)hwnd, 0, NULL);	//CreateThread creates a thread and returns handle type. 
				 //Parameters: NULL - give me default lpsecurity attributes, 0 - give me stack size same as parent's by default, associated function to run when thread starts, 4th param of CreateThread is actually the parameter passed to the function mentioned in 3rd param, 0 - thread should be started as soon as the func ends, thread id(not imp now-a-days).
	...
	case WM_LBUTTONDOWN:
		...
		break;
	case WM_DESTROY:
		CloseHandle(hThread1);						//close handle of created thread 				
...
DWORD WINAPI ThreadProcOne(LPVOID param)
{
	...
	for(i=0; i<2147483648; i++)
	{
		...
		TextOut(hdc, 5, 5, str, (int)_tcslen(str));		// (x, y)coordinates - (5,5), tcslen gives length of str which is of type TCHAR
	}
...
DWORD WINAPI ThreadProcTwo(LPVOID param)
{
	...
	for(i=2147483647; i>=0; i--)
	{
		...
		TextOut(hdc, 5, 25, str, (int)_tcslen(str));		// (x, y)coordinates - (5,25), tcslen gives length of str which is of type TCHAR
	}
	ReleaseDC((HWND)param, hdc);
	return 0;
}