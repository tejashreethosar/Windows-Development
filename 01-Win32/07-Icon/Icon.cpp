...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)		//WINAPI - __stdcall calling convention i.e pascal(r->l), hInstance - the key to uniquely identify the program, hPrevInstance - for maintaining backward compatibility, lpszCmdLine - A pointer to a char string containing the command line arguments passed to the program, iCmdShow - how the window is to be displayed : (minimized, maximized, hidden...) by default -> (SW_NORMAL)
{
	...
	wndclass.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(MYICON)); 	//I don't have my own icon, so OS give my your 'ID of an Icon which is of type application'. LoadIcon(hInstance, LPCTSTR) returns handle for icon. This icon is big icon that we see in taskbar
	...
	wndclass.hIconSm = LoadIcon(hInstance, MAKEINTRESOURCE(MYICON));	//small icon that we see in top left of window, again taken from OS
...
1. cl.exe /c /EHsc Icon.cpp
2. rc.exe Icon.rc
3. link.exe Icon.obj Icon.res user32.lin gdi32,lib
4. Icon.exe
