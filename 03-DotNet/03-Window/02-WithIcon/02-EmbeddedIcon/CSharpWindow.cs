...
public class CSharpWindow : Form
{
    ...
    {
        // code
        Icon = new Icon(GetType().Assembly.GetManifestResourceStream("BookImg.ico")); // icon of the window
...
/*
 STEPS TO RUN:
1. rc.exe Icon.rc
2. csc.exe /t:winexe /resource:BookImg.ico /win32res:Icon.res CSharpWindow.cs 
3. CSharpWindow.exe

NOTE:
This is embedded icon, so the .exe file and the .ico file need not be in the same directory because the icon gets embedded in the exe.
*/
